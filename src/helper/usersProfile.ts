...
@Injectable() 
  export class imageUploadProfile {
    ...
    async uploadUserImage(base64: string): Promise<{profilePhoto:string,imageUrl:string}>  {
      const userId = uuidv4();
      const params: AWS.S3.PutObjectRequest = {
        Bucket: process.env.AWS_BUCKET_NAME+`/userProfiles`,
      ...
      // Assuming you're saving 'photo.key' in 'photo' column and 'photo.location' in 'url' column
      const savedPhoto = {
      ...
      return savedPhoto;
        ...
        // return params.Key; // Returning the S3 path
      } catch (e) {
