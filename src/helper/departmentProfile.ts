...
@Injectable()
export class deptProfileUpload {
  private s3: AWS.S3;
  ...
  async uploadDeptImage(base64: string): Promise<{profilePhoto:string,imageUrl:string}>  {
    const userId = uuidv4();
    const params: AWS.S3.PutObjectRequest = {
      Bucket: process.env.AWS_BUCKET_NAME+`/departmentProfiles`,
